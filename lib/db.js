    ...
    return {
        ...
        parseWhere: function(where) {
            ...
            if (_.isString(where)) {
                ...
                _.forOwn(where, function(value, field) {
                    field = field.trim();
                    if (!/^[A-Z_\|\&\-.a-z0-9\(\)\,]+$/.test(field)) {
                        throw new errors.DbError('Expression error, key: [' + field + '].');
                    }

                    whereStr += (self.parseWhereItem(field, value) + operator);
                });
                ...
                _.forOwn(value, function(val, lk) {
                    ...
                    if (_.inArray(lk, ['AND', 'OR', 'XOR'])) {
                        if (_.isArray(val)) {
                            ...
                            likes[lk].push(field + ' ' + operator + ' ' + this.parseValue(val));
                        }
                    }
                });
        ...
        parseIn: function(field, operator, value) {
            ...
            if (_.isString(value) || _.isArray(value)) {
                ...
                inStr += field + ' ' + operator + ' (' + this.parseValue(value).join(',') + ')';
        ...
        parseLimit: function(limit) {
            return _.isString(limit) || _.isNumber(limit) ? ' LIMIT ' + limit : '';
        },
